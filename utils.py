...
def load_data(input_path, mode, n_steps_encoder, n_steps_decoder):
    ...
    print(global_inp_index)
    print(global_attn_index)
...
def get_valid_batch_feed_dict(model, valid_indexes, k, valid_data, global_inputs, global_attn_states):
    ...
    feed_dict = {model.phs['local_inputs']: batch_local_inp,
                 ...
                 model.phs['external_inputs']: batch_ext_inp,
                 model.phs['labels']: batch_labels}
    print('batch_local_inp=', batch_local_inp.shape)
    print('tmp=', tmp.shape)
    print('np.swapaxes(batch_local_inp, 1, 2)=', np.swapaxes(batch_local_inp, 1, 2).shape)
    print('global_attn_states[batch_global_attn]=', global_attn_states[batch_global_attn].shape)
    print('batch_ext_inp=', batch_ext_inp.shape)
    print('batch_labels=', batch_labels.shape)

    return feed_dict


def my_get_batch_feed_dict(model, k, batch_size, training_data):
    """ get feed_dict of each batch in a training epoch"""

    batch_local_inp = training_data[0][k:k + batch_size]
    tmp = training_data[1][k:k + batch_size]
    batch_global_attn_states = training_data[2][k:k + batch_size]
    batch_ext_inp = training_data[3][k:k + batch_size]
    batch_labels = training_data[4][k:k + batch_size]
    feed_dict = {model.phs['local_inputs']: batch_local_inp,
                 model.phs['global_inputs']: tmp,
                 model.phs['local_attn_states']: np.swapaxes(batch_local_inp, 1, 2),
                 model.phs['global_attn_states']: batch_global_attn_states,
                 model.phs['external_inputs']: batch_ext_inp,
                 model.phs['labels']: batch_labels}
...
def my_get_valid_batch_feed_dict(model, valid_indexes, k, valid_data):
    """ get feed_dict of each batch in the validation set"""

    valid_local_inp = valid_data[0]
    valid_global_inp = valid_data[1]
    valid_global_attn_states = valid_data[2]
    valid_ext_inp = valid_data[3]
    valid_labels = valid_data[4]
    n_steps_encoder = valid_local_inp.shape[1]
    ...
    batch_global_inp = valid_global_inp[valid_indexes[k]:valid_indexes[k + 1]]
    batch_global_attn_states = valid_global_attn_states[valid_indexes[k]:valid_indexes[k + 1]]
    feed_dict = {model.phs['local_inputs']: batch_local_inp,
                 model.phs['global_inputs']: batch_global_inp,
                 model.phs['local_attn_states']: np.swapaxes(batch_local_inp, 1, 2),
                 model.phs['global_attn_states']: batch_global_attn_states,
                 model.phs['external_inputs']: batch_ext_inp,
                 model.phs['labels']: batch_labels}
